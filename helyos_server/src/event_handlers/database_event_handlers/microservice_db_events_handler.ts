...
function processMicroserviceEvents(channel: string, payload: any) {
    let service_request_id: number; let service_request_status: string;
                ...
                databaseServices.work_processes.get_byId(payload['work_process_id'], ['status'])
                    .then(work_process => {
                        if ([MISSION_STATUS.FAILED,
                            ...
                            MISSION_STATUS.PLANNING_FAILED].includes(work_process.status)) {
                            return databaseServices.service_requests.update_byId(service_request_id, {
                ...
                case SERVICE_STATUS.READY:
                    if (payload['is_result_assignment'] && !payload['assignment_dispatched']) {
                        microResp.processMicroserviceResponse(payload)
                            ...
                            .catch(err => {
                                databaseServices.service_requests.update_byId(payload['id'], {
