...
const moveMemDataToDB = async () => {
    ...
    if (nodeLeader.amILeader){
        ...
        inMemDB.flush('agents', 'uuid', databaseServices.agents, 0).catch(error => console.log("push cache to db", error));
        inMemDB.flush('map_objects', 'id', databaseServices.map_objects, 0).catch(error => console.log("push cache to db", error));
    }
};
