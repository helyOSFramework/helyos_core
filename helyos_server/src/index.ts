...
// ----------------------------------------------------------------------------
const {
    handleDatabaseMessages,
} = require('./event_handlers/database_event_subscriber.js');
...
const setDashboardServer = () => {
    const pubKey = fs.readFileSync('/etc/helyos/.ssl_keys/helyos_public.key',{
        encoding: 'utf8',
        flag: 'r',
    });
    const app = express();
...
const {
    DASHBOARD_PORT, SOCKET_PORT, GQLPORT, SOCKET_IO_ADAPTER,
} = config;
