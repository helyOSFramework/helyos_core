...
const {
    MESSAGE_RATE_LIMIT, MESSAGE_UPDATE_LIMIT,
    AGENT_IDLE_TIME_OFFLINE, DB_BUFFER_TIME,
} = config;
...
function createOrUpdateAgents(fileNames) {
    const promises = fileNames.map(fileName => {
        ...
        const pubKey = fs.readFileSync(`/etc/helyos/.ssl_keys/agent_public_keys/${fileName}`, {
            encoding: 'utf8',
            flag: 'r',
        });
        return databaseServices.agents.get('uuid', uuid)
            .then(agents => {
                if (agents.length > 0) {
                    return databaseServices.agents.update('id', agents[0].id, {
                        public_key: pubKey,
                    });
                } else {
...
export async function helyosConsumingMessages(dataChannels) {
    ...
    mainChannel.consume(CHECK_IN_QUEUE, (message) => handleBrokerMessages(mainChannel, CHECK_IN_QUEUE, message), {
        noAck: true,
        priority: 5,
    });
    mainChannel.consume(AGENT_MISSION_QUEUE, (message) => handleBrokerMessages(mainChannel, AGENT_MISSION_QUEUE, message), {
        noAck: true,
        priority: 5,
    });
    mainChannel.consume(SUMMARY_REQUESTS_QUEUE, (message) => handleBrokerMessages(mainChannel, SUMMARY_REQUESTS_QUEUE, message), {
        noAck: true,
    });
