...
const parseConditions = (tableName:string, conditions:any) => {
	...
	let values: string[] = [];
	let masksArray: string[] = [];
	let null_conditionsArray: string[] = [];
	let in_conditionsArray: string | string[] = [];

	if (!conditions || Object.keys(conditions).length == 0) {
		return {};
	}

	let fromTableStatementsArray: string[] = [];
...
class DatabaseLayer {
	...
	insert(patch:any, useShortTimeClient = false) {
		const colNamesArray:string[] = [], colValues: any[] = [], valueMasksArray:string[] = [];
		delete patch['id'];
	...
	updateByConditions(conditions, patch, useShortTimeClient = false) {
		...
		let colNamesArray:string[] = [], colValues = [...condColValues!], valueMasksArray:string[] = [];
		delete patch['id'];
	...
	delete(conditions, useShortTimeClient = false) {
		let colNamesArray:string[] = [], colValues:string[] = [], valueMasksArray:string[] = [];
		let null_conditionsArray:string[] = [];
		if (!conditions || Object.keys(conditions).length == 0) {
			return this.list(useShortTimeClient);
		}
...
export { DatabaseLayer, AgentDataLayer, Client,
