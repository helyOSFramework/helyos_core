...
const guestCreate_RbmqAdmin = (username: string, password: string, tags: string[]): Promise<{ logType: string; message: string }> =>
    requestXHTTP
        ...
        .then(() => ({
            logType: 'info',
            message: `RabbitMQ admin created for ${username}`,
        }))
        .catch(e => {
...
const guestAdd_RbmqAdminVhost = (username: string): Promise<{ logType: string; message: string }> =>
    !username
        ...
        : requestXHTTP
            ...
            .then(() => ({
                logType: 'info',
                message: `Set ${username} permissions in RabbitMQ/${RBMQ_VHOST}`,
            }))
            .catch(e => {
...
const createUser = (username: string, password: string, tags: string[]): Promise<{ logType: string; message: string}> =>
    !username
        ...
        : requestXHTTP
            ...
            .then(() => ({
                logType: 'info',
                message: `RabbitMQ account created for ${username}`,
            }))
            .catch(e => {
...
const removeUser = (username: string): Promise<{ logType: string; message: string } | null> =>
    !username
        ...
        : requestXHTTP
            ...
            .then(() => ({
                logType: 'info',
                message: `RabbitMQ ${username} account deleted`,
            }))
            .catch(e => {
                throw new Error(`DELETE ACCOUNT RMBTMQ: ${e.response?.body?.reason}`);
            });

const addRbmqUserVhost = (username: string, permissions: Permissions = {
    configure: '.*',
    write: '.*',
    read: '.*',
}): Promise<{ logType: string; message: string } | null> =>
    !username
        ? Promise.resolve(null)
        : requestXHTTP
            .put(`${API_PROTOCOL}://${RBMQ_HOST}:${RBMQ_API_PORT}/api/permissions/${RBMQ_VHOST}/${username}`)
            ...
            .then(() => ({
                logType: 'info',
                message: `Set account permissions within RabbitMQ-vhost:${RBMQ_VHOST}`,
            }))
            .catch(e => {
                throw new Error(`VHOST PERMISSION: ${e.response?.body?.reason}`);
            });
    ...
    !username
        ...
        : requestXHTTP
            ...
            .send({
                ...
                write: guestPermissions,
                read: guestPermissions,
            })
            .set({
                'content-type': 'application/json',
            })
            ...
            .then(() => ({
                logType: 'info',
                message: `Set anonymous agent permissions in RabbitMQ`,
            }))
            .catch(e => {
